...
| 7 | First normalized sample ingestion — planned | Wearable-sourced `MetricEntry` rows | `SyncRun` reaches a terminal status; `CONN_1` becomes `connected` and receives `last_synced_at` after successful ingestion | Source record IDs are deduplicated and the imported/skipped counters describe the result |
...
- The `SyncRun` receipt model, database idempotency constraint, and receipt-only upload endpoint are implemented. Automatic `MetricEntry` ingestion remains planned.
