workspace "SyncVitals Current AWS V017" "Verified presentation-staging state after public Stripe reconciliation and a successful PostgreSQL backup/restore drill on 2026-09-15." {
    model {
            currentAwsV017 = deploymentEnvironment "[CURRENT AWS V017] Staging Backend Live" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation staging. CloudFront serves the private S3 frontend and guarded Django API. One ARM64 EC2 host runs Django and PostgreSQL with retained EBS data. Host metrics, Stripe reconciliation, and one S3 backup/restore drill are verified; scheduled backups and alarms remain pending." {
                tags "CloudZone"
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend assets, backend images, runtime secret, monitoring, and PostgreSQL backups." {
                    ...
                    backupBucket = infrastructureNode "Private PostgreSQL Backups" "syncvitals-staging-backups-173291122778-eu-central-1-an: ACLs disabled, all public access blocked, SSE-S3, versioning, and HTTPS-only enforcement. One 82,075-byte custom dump has verified checksum, object version, and disposable restore evidence. Scheduling, retention, and backup alarms remain pending." {
                        tags "StorageService"
                    }
            ...
            databaseContainer -> backupBucket "Produced one validated custom-format logical dump" "pg_dump -> HTTPS PutObject" {
                tags "StorageTraffic"
            }
    ...
    views {
        deployment * currentAwsV017 "current-aws-v017" "[CURRENT AWS V017] Actual state: V016 plus verified Stripe test Checkout, Portal, signed webhook reconciliation and cancellation state, together with a private encrypted versioned S3 backup bucket and one checksum-verified PostgreSQL dump restored into an isolated disposable database. Daily scheduling, retention, backup monitoring, logs, alarms, and Android end-to-end flows remain pending." {
            include *
