workspace "SyncVitals Current AWS V009" "Verified presentation-staging deployment state after creating the CloudFront-only origin security group on 2026-09-06." {
    model {
        currentAwsV009 = deploymentEnvironment "[CURRENT AWS V009] Origin Security Group" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. Frontend delivery, the accepted backend image, the validated runtime secret, the restricted EC2 instance role, and the CloudFront-only origin security group exist; no EC2, EBS, Elastic IP, Nginx, Django, PostgreSQL, or staging CloudWatch resource exists yet." {
                tags "CloudZone"
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing 172.31.0.0/16 network; no SyncVitals compute exists yet." {
                        tags "NetworkZone"

                        originSecurityGroup = infrastructureNode "syncvitals-staging-origin" "Security group sg-0bb8f60ee0b21cb06 allows inbound TCP 443 only from pl-a3a144ca. It has no inbound SSH, HTTP, Gunicorn, PostgreSQL, IPv4 CIDR, or IPv6 CIDR rule; default all-IPv4 outbound remains enabled for host dependencies." {
                            tags "SecurityService"
                        }
                    }
    ...
    views {
        deployment * currentAwsV009 "current-aws-v009" "[CURRENT AWS V009] Actual state: V008 plus the unattached CloudFront-only origin security group; no EC2 host or API compute exists yet." {
            include *
