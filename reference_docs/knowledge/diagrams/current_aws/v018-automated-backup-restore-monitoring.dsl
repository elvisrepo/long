workspace "SyncVitals Current AWS V018" "Verified presentation-staging state after automated PostgreSQL backups, monthly restore checks, and email-tested backup monitoring on 2026-09-17." {
    model {
            currentAwsV018 = deploymentEnvironment "[CURRENT AWS V018] Staging Backend Live" {
            userDevices = deploymentNode "User Devices" "Presentation clients that resolve the public staging hostname." {
                tags "ClientZone"

                browser = infrastructureNode "Web Browser" "Loads the React application and has completed verified sign-in, Stripe Checkout, Portal, and cancellation flows through staging.syncvitals.space." {
                    tags "ClientRuntime"
                }
                ...
                operatorEmail = infrastructureNode "Operator Email Inbox" "Confirmed subscriber for staging operational alerts; address omitted." {
                    tags "ClientRuntime"
                }
            }
            ...
            stripe = deploymentNode "Stripe Test Mode" "External sandbox billing provider used by presentation staging; no live-mode money or production customer data is involved." {
                tags "ExternalZone"

                stripeBilling = infrastructureNode "Stripe Checkout + Customer Portal" "Hosts the verified Pro monthly Checkout, billing-management, and period-end cancellation flows." {
                    tags "ExternalService"
                }

                stripeWebhook = infrastructureNode "Registered Staging Webhook" "Sends signed checkout.session.completed, customer.subscription.updated, and customer.subscription.deleted events to the public staging API." {
                    tags "ExternalService"
                }
            }

            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned staging: CloudFront serves the private frontend and guarded Django API. One ARM64 EC2 host runs PostgreSQL on retained EBS. Daily S3 backups, monthly isolated restore checks, and email-tested alarms are active." {
                tags "CloudZone"
                ...
                iam = deploymentNode "IAM (Global)" "Identity and least-privilege permissions for the staging host." {
                    ...
                    backupWritePolicy = infrastructureNode "SyncVitalsStagingDatabaseBackupWrite" "Inline policy permits PutObject and AbortMultipartUpload only below the backup bucket's postgresql/ prefix. It grants no read, list, delete, or bucket-administration access." {
                        tags "PolicyService"
                    }

                    backupReadPolicy = infrastructureNode "SyncVitalsStagingDatabaseBackupRestoreRead" "Lists only postgresql/ and reads objects only under that prefix; no delete or bucket administration." {
                        tags "PolicyService"
                    }
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend assets, backend images, runtime secret, monitoring, and PostgreSQL backups." {
                    tags "CloudZone"
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing VPC vpc-08d84eb2a3aa76777 with CIDR 172.31.0.0/16; it now contains the one SyncVitals presentation-staging host." {
                        ...
                        deploymentBundle = infrastructureNode "Deployment Bundle" "Root-owned Compose and storage-guard configuration under /opt/syncvitals/deployment. Backup and restore automation is installed separately on the host." {
                        ...
                        databaseContainer = infrastructureNode "PostgreSQL 16 Container" "syncvitals-staging-database-1 is healthy and exposes no host or public port. It holds the verified staging user, metric, subscription, Checkout, and Stripe webhook records." {
                        ...
                        backupTimer = infrastructureNode "Daily Backup Job" "Enabled systemd timer at 03:15 UTC with up to 30 minutes delay. Latest scheduled run on 2026-09-17 uploaded a PostgreSQL dump." {
                            tags "HostAutomationService"
                        }

                        restoreCheck = infrastructureNode "Monthly Restore Check" "Enabled timer on day 1 at 04:15 UTC with up to 45 minutes delay. Verifies checksum, disposable restore, required tables, and migrations; manual run passed." {
                            tags "HostAutomationService"
                        }

                        successMarker = infrastructureNode "Restore Success Month" "Root-only 0600 file records the UTC month of the last successful isolated restore: 2026-09." {
                            tags "HostAutomationService"
                        }

                        restoreHeartbeat = infrastructureNode "Restore Freshness Job" "Enabled six-hour timer publishes 1 for a current-month restore, else 0. The prior month counts until 06:00 UTC on day 1." {
                            tags "HostAutomationService"
                        }
                    }

                    cloudWatch = deploymentNode "Amazon CloudWatch" "Frankfurt host, backup, and restore metrics with verified alarms. Application logs and traces remain unconfigured." {
                        tags "CloudZone"
                        ...
                        backupMetrics = infrastructureNode "Backup Result Metric" "CWAgent/StagingDatabaseBackupSuccess: 1 for success, 0 for failure; InstanceId dimension." {
                            tags "MonitoringService"
                        }

                        restoreMetrics = infrastructureNode "Restore Metrics" "CWAgent/StagingDatabaseRestoreSuccess and StagingDatabaseRestoreFresh; InstanceId dimension." {
                            tags "MonitoringService"
                        }

                        backupAlarms = infrastructureNode "Backup Health Alarms" "Failure and overdue daily-run alarms feed an Unhealthy composite. All were OK; ALARM and OK actions notify SNS." {
                            tags "MonitoringService"
                        }

                        restoreAlarms = infrastructureNode "Restore Health Alarms" "Explicit failure and overdue freshness alarms. Both were OK; ALARM and OK actions notify SNS." {
                            tags "MonitoringService"
                        }
                    }

                    sns = deploymentNode "Amazon SNS" "Regional operational notifications in Frankfurt." {
                        tags "CloudZone"

                        alertsTopic = infrastructureNode "syncvitals-staging-alerts" "One confirmed email subscriber; controlled missing-heartbeat alert was received." {
                            tags "MonitoringService"
                        }
                    }
                    ...
                    backupBucket = infrastructureNode "Private PostgreSQL Backups" "Private versioned SSE-S3 bucket with HTTPS-only access. Daily postgresql/ dumps; current versions expire after 90 days, noncurrent after 30, incomplete uploads after 7; expired delete markers are cleaned." {
                        tags "StorageService"
                    }
                    ...
                    secretsManager = deploymentNode "Secrets Manager" "Regional encrypted configuration store for the staging runtime." {
                        ...
                        runtimeSecret = infrastructureNode "longevity/staging/backend-runtime" "AWSCURRENT contains validated application runtime fields, the CloudFront origin header, and the Stripe webhook signing secret. Values are omitted; automatic rotation is not configured." {
                            tags "SecurityService"
                        }
                    }
                }
            ...
            instanceRole -> backupWritePolicy "Has write-only PostgreSQL backup permission" "Inline IAM policy" {
                tags "IdentityTraffic"
            }
            instanceRole -> backupReadPolicy "Has prefix-scoped restore-check read permission" "Inline IAM policy" {
                tags "IdentityTraffic"
            }
            backupReadPolicy -> backupBucket "Allows list and GetObject only below postgresql/" "IAM authorization" {
                tags "IdentityTraffic"
            }
            backupWritePolicy -> backupBucket "Allows encrypted uploads only below postgresql/" "IAM authorization" {
                tags "IdentityTraffic"
            }
            metricsWritePolicy -> hostMetrics "Allows publishing only to CWAgent in Frankfurt" "IAM authorization" {
                tags "IdentityTraffic"
            }
            metricsWritePolicy -> backupMetrics "Allows backup result metric publication" "IAM authorization" {
                tags "IdentityTraffic"
            }
            metricsWritePolicy -> restoreMetrics "Allows restore result and freshness metric publication" "IAM authorization" {
                tags "IdentityTraffic"
            }
            ...
            backupTimer -> databaseContainer "Reads a consistent logical PostgreSQL snapshot" "docker exec / pg_dump" {
                tags "StorageTraffic"
            }
            backupTimer -> backupBucket "Uploads dated, checksum-tagged custom dumps daily" "HTTPS PutObject" {
                tags "StorageTraffic"
            }
            backupTimer -> backupMetrics "Publishes 1 on success or 0 on failure" "CloudWatch PutMetricData" {
                tags "MonitoringTraffic"
            }
            restoreCheck -> backupBucket "Reads and verifies the newest dump" "S3 list + GetObject" {
                tags "StorageTraffic"
            }
            restoreCheck -> dockerRuntime "Starts a disposable PostgreSQL restore container" "Docker API" {
                tags "AutomationTraffic"
            }
            restoreCheck -> restoreMetrics "Publishes isolated restore result" "CloudWatch PutMetricData" {
                tags "MonitoringTraffic"
            }
            restoreCheck -> successMarker "Records last successful UTC month" "Atomic root-only file write" {
                tags "AutomationTraffic"
            }
            restoreHeartbeat -> successMarker "Reads the recorded month every six hours" "Root-only file read" {
                tags "AutomationTraffic"
            }
            restoreHeartbeat -> restoreMetrics "Publishes fresh=1 or stale=0" "CloudWatch PutMetricData" {
                tags "MonitoringTraffic"
            }
            backupMetrics -> backupAlarms "Drives failed and overdue backup checks" "CloudWatch alarm evaluation" {
                tags "MonitoringTraffic"
            }
            restoreMetrics -> restoreAlarms "Drives failed and overdue restore checks" "CloudWatch alarm evaluation" {
                tags "MonitoringTraffic"
            }
            backupAlarms -> alertsTopic "Sends unhealthy and recovery notifications" "SNS alarm action" {
                tags "MonitoringTraffic"
            }
            restoreAlarms -> alertsTopic "Sends failure, overdue, and recovery notifications" "SNS alarm action" {
                tags "MonitoringTraffic"
            }
            alertsTopic -> operatorEmail "Delivers staging alert email; test received" "SNS email subscription" {
                tags "MonitoringTraffic"
            }
            ...
            apiContainer -> stripeBilling "Creates Checkout and Portal sessions and reconciles billing state" "Stripe test HTTPS API" {
                tags "ApplicationTraffic"
            }
            ec2Host -> dockerRuntime "Runs ARM64 Docker" "Host service" {
                tags "HostTraffic"
            }
            ec2Host -> backupTimer "Runs the daily backup timer" "systemd" {
                tags "AutomationTraffic"
            }
            ec2Host -> restoreCheck "Runs the monthly restore timer" "systemd" {
                tags "AutomationTraffic"
            }
            ec2Host -> restoreHeartbeat "Runs the six-hour freshness timer" "systemd" {
                tags "AutomationTraffic"
            }
            ...
            browser -> stripeBilling "Completes hosted test Checkout and Portal journeys" "HTTPS" {
                tags "ClientTraffic"
            }
            ...
            stripeBilling -> stripeWebhook "Emits configured billing events" "Stripe event delivery" {
                tags "ApplicationTraffic"
            }
            stripeWebhook -> distribution "Delivers signed events to the staging webhook route" "HTTPS POST /api/v1/subscriptions/webhooks/stripe/" {
                tags "ClientTraffic"
            }
    ...
    views {
        deployment * currentAwsV018 "current-aws-v018" "[CURRENT AWS V018] V017 plus daily S3 PostgreSQL backups, bounded retention, monthly isolated restore verification, six-hour freshness checks, and email-tested CloudWatch/SNS alarms. Application logs, traces, and authenticated Android end-to-end flows remain pending." {
            include *
            autolayout lr 180 110
        }

        deployment * currentAwsV018 "current-aws-v018-backup-operations" "[CURRENT AWS V018 / FOCUSED] Daily pg_dump to private S3; monthly disposable restore; six-hour freshness heartbeat; backup and restore alarms notify the confirmed operator email." {
            include databaseContainer
            include backupTimer
            include backupBucket
            include restoreCheck
            include successMarker
            include restoreHeartbeat
            include backupMetrics
            include restoreMetrics
            include backupAlarms
            include restoreAlarms
            include alertsTopic
            include operatorEmail
            autolayout lr 180 120
        }
