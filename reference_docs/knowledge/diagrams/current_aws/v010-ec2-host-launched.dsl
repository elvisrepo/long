workspace "SyncVitals Current AWS V010" "Verified presentation-staging deployment state after launching and inspecting the EC2 origin host on 2026-09-06." {
    model {
        currentAwsV010 = deploymentEnvironment "[CURRENT AWS V010] EC2 Origin Host Launched" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. Frontend delivery and the backend prerequisites now include one running EC2 origin host with an encrypted root disk and working Systems Manager access. The host software, persistent database volume, stable origin address, Nginx, Django, PostgreSQL, and staging CloudWatch resources do not exist yet." {
                tags "CloudZone"
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing VPC vpc-08d84eb2a3aa76777 with CIDR 172.31.0.0/16; it now contains the one SyncVitals presentation-staging host." {
                        ...
                        ec2Host = infrastructureNode "EC2 Origin Host" "Running instance i-08fbc9f0c53265b63 is a healthy t4g.small ARM64 host in eu-central-1c on Canonical Ubuntu 24.04. The intended role and security group are attached, IMDSv2 is required, termination protection is enabled, and Session Manager access works. Docker and the remaining host software are not installed yet." {
                            tags "ComputeService"
                        }
                    }
    ...
    views {
        deployment * currentAwsV010 "current-aws-v010" "[CURRENT AWS V010] Actual state: V009 plus one healthy ARM64 EC2 origin host, encrypted root EBS, attached restricted role/security group, and verified SSM access. Host bootstrap and the separate PostgreSQL volume remain incomplete." {
            include *
