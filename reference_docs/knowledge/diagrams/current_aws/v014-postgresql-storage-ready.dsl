workspace "SyncVitals Current AWS V014" "Verified presentation-staging state after preparing the encrypted PostgreSQL EBS volume and verifying retention and automatic mounting after reboot on 2026-09-09." {
    model {
        currentAwsV014 = deploymentEnvironment "[CURRENT AWS V014] PostgreSQL Storage Ready" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. Frontend delivery and backend prerequisites include one updated EC2 origin host with an encrypted root disk, working Systems Manager access, a verified native ARM64 Docker runtime, a verified Nginx service, verified Certbot DNS-01 tooling with an active renewal timer, and CloudWatch Agent publishing memory and root-disk metrics. A separate encrypted 10 GiB database volume is attached, formatted, mounted, and verified after reboot. The stable origin address, issued origin TLS certificate, Django and PostgreSQL containers, and CloudFront API origin do not exist yet. CloudWatch logs, traces, and alarms are not configured." {
                tags "CloudZone"
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing VPC vpc-08d84eb2a3aa76777 with CIDR 172.31.0.0/16; it now contains the one SyncVitals presentation-staging host." {
                        ...
                        ec2Host = infrastructureNode "EC2 Origin Host" "i-08fbc9f0c53265b63: healthy t4g.small ARM64 in eu-central-1c. Updated Ubuntu 24.04; kernel 7.0.0-1012-aws. Role and security group attached; IMDSv2 required; termination protection on; Session Manager verified." {
                            tags "ComputeService"
                        }

                        dockerRuntime = infrastructureNode "Docker Engine + Compose" "ARM64 Docker 29.8.0; containerd 2.3.4; Compose 5.5.1; Buildx 0.37.0. Docker active and boot-enabled. ARM64 hello-world passed. No application containers deployed." {
                            tags "RuntimeService"
                        }

                        postgresImage = infrastructureNode "Cached PostgreSQL 16 Image" "Cached postgres:16, digest sha256:f1c3376c26f2609ab9f29f71f824103fe2fcd8ee0346485cb6122a4f93df6f94. Temporary id-only container verified UID/GID 999:999, then removed. Database not initialized or running." {
                            tags "AcceptedArtifact"
                        }

                        databaseMount = infrastructureNode "Database Filesystem" "/srv/syncvitals: ext4, label staging-postgres. UUID f4a12602-0ab0-45ae-a73d-dc6fc8fb00e2. /dev/nvme1n1; fstab defaults,nofail; fsck pass 2. Reboot remount verified. PostgreSQL must require this mount." {
                            tags "StorageService"
                        }

                        postgresDirectory = infrastructureNode "PostgreSQL Data Directory" "/srv/syncvitals/postgresql: empty directory on the database EBS disk. Owner 999:999 matches the image's postgres user. Mode 700: owner-only access, with root override. Permissions verified after reboot; no database initialized." {
                            tags "StorageService"
                        }

                        nginx = infrastructureNode "Nginx 1.24.0" "Active and boot-enabled; syntax and local HTTP 200 verified. Packaged port-80 site only; security group blocks port 80. Origin TLS, reverse proxy, and CloudFront API origin await configuration." {
                        ...
                        certbot = infrastructureNode "Certbot 2.9.0 + dns-route53" "Certbot 2.9.0 and Route 53 DNS-01 plugin installed and discovered. No certificate requested, ACME TXT changed, or Nginx TLS configured." {
                            tags "SecurityService"
                        }

                        certbotTimer = infrastructureNode "Certbot Renewal Timer" "systemd timer active and boot-enabled. Scheduled activation verified for 2026-09-07 18:01:38 UTC. No certificate exists to renew yet." {
                    ...
                    rootVolume = infrastructureNode "Encrypted EC2 Root Volume" "vol-0fb2e65033bb83fb5: 16 GiB gp3, encrypted with aws/ebs. DeleteOnTermination=true. Stores Ubuntu, software, and cached Docker images. Database files use the separate 10 GiB disk." {
                        tags "StorageService"
                    }
            ...
            dockerRuntime -> postgresImage "Caches the pinned image; id-only verification completed" "Local OCI image" {
                tags "HostTraffic"
            }
            ...
            certbotTimer -> certbot "Invokes periodic renewal checks; no certificate exists yet" "systemd timer" {
    ...
    views {
        deployment * currentAwsV014 "current-aws-v014" "[CURRENT AWS V014] Actual state: V013 plus the encrypted 10 GiB PostgreSQL EBS volume, ext4 mount, and prepared data directory. Retention and automatic remount after reboot are verified. PostgreSQL has not been initialized or started; database-disk monitoring and the deployment mount guard remain pending." {
            include *
