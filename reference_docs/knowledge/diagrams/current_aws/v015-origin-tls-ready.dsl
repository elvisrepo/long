workspace "SyncVitals Current AWS V015" "Verified presentation-staging state after assigning a stable origin address, issuing the origin certificate, and configuring Nginx TLS on 2026-09-09." {
    model {
            currentAwsV015 = deploymentEnvironment "[CURRENT AWS V015] Origin TLS Ready" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. Frontend delivery and backend prerequisites include one updated EC2 origin host with an encrypted root disk, working Systems Manager access, a verified native ARM64 Docker runtime, Nginx TLS, Certbot DNS-01 renewal, and CloudWatch Agent publishing memory and root-disk metrics. A separate encrypted 10 GiB database volume is attached, formatted, mounted, and verified after reboot. The Django and PostgreSQL containers and CloudFront API origin do not exist yet. CloudWatch logs, traces, and alarms are not configured." {
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing VPC vpc-08d84eb2a3aa76777 with CIDR 172.31.0.0/16; it now contains the one SyncVitals presentation-staging host." {
                        ...
                        nginx = infrastructureNode "Nginx 1.24.0" "Active and boot-enabled. HTTP redirects to HTTPS; port 443 serves origin-staging.syncvitals.space with the Certbot certificate. The current HTTPS root intentionally returns 404 until the API is deployed; reverse proxy and CloudFront API origin remain pending." {
    ...
    views {
        deployment * currentAwsV015 "current-aws-v015" "[CURRENT AWS V015] Actual state: V014 plus the stable origin EIP/DNS record, issued Let’s Encrypt certificate, renewal hook, and Nginx HTTP-to-HTTPS/TLS listener. The HTTPS root remains a deliberate 404 until the API is deployed; PostgreSQL and the CloudFront API origin remain pending." {
            include *
