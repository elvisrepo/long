workspace "SyncVitals Current AWS V005" "Verified presentation-staging deployment state after creating the private backend ECR repository on 2026-09-04." {
    model {
        currentAwsV005 = deploymentEnvironment "[CURRENT AWS V005] Private Backend ECR" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. The private backend ECR repository now exists; no EC2, EBS, Elastic IP, Secrets Manager, Nginx, Django, PostgreSQL, or staging CloudWatch resource exists yet." {
                tags "CloudZone"
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    ecr = deploymentNode "Elastic Container Registry" "Private regional OCI image registry for the presentation-staging backend." {
                        ...
                        backendRepository = infrastructureNode "syncvitals/staging/backend" "Empty private repository. Image tags are immutable, AES-256 encryption is enabled, basic scanning runs on push, and no lifecycle or repository policy exists yet." {
    ...
    views {
        deployment * currentAwsV005 "current-aws-v005" "[CURRENT AWS V005] Actual state: V004 plus the empty private backend ECR repository with immutable tags, AES-256 encryption, and basic scan on push; no API compute exists yet." {
            include *
