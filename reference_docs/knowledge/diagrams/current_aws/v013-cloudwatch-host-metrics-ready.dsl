workspace "SyncVitals Current AWS V013" "Verified presentation-staging state after console installation of CloudWatch Agent and verified memory and root-disk metric delivery on 2026-09-08." {
    model {
        currentAwsV013 = deploymentEnvironment "[CURRENT AWS V013] CloudWatch Host Metrics Ready" {
            ...
            aws = deploymentNode "AWS Account 173291122778" "Manually provisioned presentation-staging account. Frontend delivery and backend prerequisites include one updated EC2 origin host with an encrypted root disk, working Systems Manager access, a verified native ARM64 Docker runtime, a verified Nginx service, verified Certbot DNS-01 tooling with an active renewal timer, and CloudWatch Agent publishing memory and root-disk metrics. The persistent database volume, stable origin address, issued origin TLS certificate, Django and PostgreSQL containers, and CloudFront API origin do not exist yet. CloudWatch logs, traces, and alarms are not configured." {
                ...
                euCentral1 = deploymentNode "eu-central-1 (Frankfurt)" "Chosen region for the application host, frontend object storage, and backend container registry." {
                    ...
                    defaultVpc = deploymentNode "Default VPC" "Existing VPC vpc-08d84eb2a3aa76777 with CIDR 172.31.0.0/16; it now contains the one SyncVitals presentation-staging host." {
                        ...
                        cloudWatchAgent = infrastructureNode "CloudWatch Agent" "Installed and configured through the CloudWatch console. Runs as cwagent and collects mem_used_percent and disk_used_percent for / every 60 seconds. Metric delivery is verified; exact installed version and boot enablement have not been inspected. Workload detection is disabled." {
                            tags "MonitoringService"
                        }

                        certbot = infrastructureNode "Certbot 2.9.0 + dns-route53" "Certbot and the Route 53 DNS-01 authenticator are installed and plugin-discovered. No certificate has been requested, no ACME TXT record has been changed, and no Nginx TLS configuration exists yet." {
                            tags "SecurityService"
                        }

                        certbotTimer = infrastructureNode "Certbot Renewal Timer" "The systemd timer is enabled and active. Its next verified activation was scheduled for 2026-09-07 18:01:38 UTC; without an issued certificate, there is currently nothing to renew." {
                    ...
                    cloudWatch = deploymentNode "Amazon CloudWatch" "Regional host monitoring in Frankfurt. Memory and root-disk metric datapoints are verified; logs, traces, and alarms are not configured." {
                        ...
                        hostMetrics = infrastructureNode "CWAgent Host Metrics" "Two verified series for i-08fbc9f0c53265b63: mem_used_percent with InstanceId, and disk_used_percent with InstanceId, fstype=ext4, path=/. No aggregation rollups or device dimension. Observed memory approximately 16% and root-disk usage approximately 26% on 2026-09-08." {
                            tags "MonitoringService"
                        }
                    }
            ...
            ec2Host -> certbot "Provides the installed DNS-01 certificate tooling" "Host package" {
                tags "HostTraffic"
            }
    ...
    views {
        deployment * currentAwsV013 "current-aws-v013" "[CURRENT AWS V013] Actual state: V012 plus CloudWatch Agent, scoped metric publishing permissions, and verified memory and root-disk metrics in Frankfurt. Logs, traces, and alarms remain unconfigured; origin TLS, application containers, and persistent PostgreSQL storage remain absent." {
            include *
