...
- **Wearable payload abuse or schema smuggling**: The isolated future batch contract requires `1–100` entries, rejects undeclared fields at the batch and nested-entry levels, rejects non-finite numeric values, and rejects repeated external record IDs within a batch.
- **Wearable upload identity reused for different content**: The server, rather than the Android client, computes a versioned canonical SHA-256 payload fingerprint after validation. Entry order and equivalent timezone representations do not change the fingerprint; changed, added, or removed entries do. The helper is implemented, but comparison with the stored `SyncRun.payload_hash` is not yet wired into the receipt-only endpoint.
