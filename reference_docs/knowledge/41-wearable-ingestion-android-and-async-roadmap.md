...
- `LongevityApplication` is the minimal application-level dependency container: it shares one OkHttp client, one `HttpAuthRepository`, one `AuthenticatedApiClient`, the connection/upload repositories, one dual-purpose Health Connect access/reader adapter, the initial sync coordinator, and the Android-Keystore token store without introducing a dependency-injection framework prematurely.
