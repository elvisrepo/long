...
- Django now seeds `steps` as a system activity metric (`0` through `200000` steps per entry), but the wearable upload allowlist and Android Health Connect pipeline remain weight-only until the Steps record semantics are implemented and tested.
