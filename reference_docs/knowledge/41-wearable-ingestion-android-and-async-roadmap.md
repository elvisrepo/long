...
- Django seeds `steps` as a system activity metric (`0` through `200000` steps per entry) and the shared upload endpoint now persists normalized Steps intervals. `MetricEntry.period_start` stores the interval beginning while `recorded_at` stores its end; instantaneous Weight leaves `period_start` null. Android now declares `READ_STEPS` and owns SDK-independent Steps sample/reader contracts, but its concrete Health Connect adapter and sync planner remain Weight-only until the next TDD slices.
