...
- Django seeds `steps` as a system activity metric (`0` through `200000` steps per entry) and the shared upload endpoint now persists normalized Steps intervals. `MetricEntry.period_start` stores the interval beginning while `recorded_at` stores its end; instantaneous Weight leaves `period_start` null. Android declares `READ_STEPS`, owns SDK-independent Steps sample/reader contracts, and maps every ascending Health Connect `StepsRecord` page through the shared `AndroidHealthConnectAccess`. Permission orchestration and the sync planner remain Weight-only until the next TDD slices.
