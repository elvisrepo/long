...
- Django seeds `steps` as a system activity metric (`0` through `200000` steps per entry) and the shared upload endpoint now persists normalized Steps intervals. `MetricEntry.period_start` stores the interval beginning while `recorded_at` stores its end; instantaneous Weight leaves `period_start` null. The Android Health Connect reader/planner remains Weight-only until its Steps adapter is implemented and tested.
