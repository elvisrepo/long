...
- **Implemented Android startup restoration**: `LoginViewModel` asks `AuthRepository.restoreSession()` once at creation and exposes an explicit checking state so the credential form does not flash. Restoration only checks for a readable Keystore-backed local pair; it does not rotate the refresh token merely because Android recreated the process. Token values never enter ViewModel or Compose state.
- **Implemented Android on-demand refresh**: product requests first reuse the stored access token. After Django rejects it with `401`, `AuthenticatedApiClient` calls the separate `SessionRefresher` boundary. `HttpAuthRepository.refreshSession()` posts the stored refresh token to `/api/auth/mobile/refresh/`, saves the replacement access token, and saves a rotated refresh token when Django returns one. Without rotation, it retains the existing refresh token.
- A refresh-endpoint `401` is authoritative rejection, so Android clears both local tokens and reports `NoSession`. Missing local tokens skip network requests. Transient refresh/storage failures retain readable tokens for a later retry and report `Unavailable`.
...
- Rotating refresh tokens creates a small cross-system durability window: Django invalidates the submitted token before Android can durably commit its replacement. The Android store uses synchronous `SharedPreferences.commit()`, and on-demand refresh avoids unnecessary exposure on every process start, but neither eliminates process death between the server response and local commit. Production hardening should evaluate a narrowly bounded server-side rotation grace/recovery design without allowing unrestricted replay.
