...
- The server-side canonical payload-hash helper deterministically fingerprints validated entries with schema version `1`, stable external-record ordering, UTC timestamps, and SHA-256. It is not yet wired to the live receipt endpoint; same-upload/different-payload rejection remains the next API behavior.
