...
- An isolated `WearableUploadBatchSerializer` composes `connection_id`, `upload_id`, and a required list of `1–100` normalized entries. It rejects undeclared fields at both the batch and entry levels. It is not yet used by the live receipt endpoint.
