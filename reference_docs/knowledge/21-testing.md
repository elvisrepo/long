...
- Upload API tests prove authentication, owner/active scoping, malformed UUID and missing-entry rejection, synchronous normalized entry persistence with `201`, an exact retry with `200`, both conflict types with safe `409` detail, and cross-upload identical-record skipping with terminal counters.
...
- Wearable-entry serializer tests prove normalized `body_weight` acceptance, configured range and finite-number enforcement, active supported system-definition lookup, Samsung Health source restriction, timestamp parsing, and required nonblank external IDs. This serializer is the live nested-entry boundary.
...
Current Android testing checkpoint — 2026-08-08:
...
- `WeightSyncSchedulerTest` proves each periodic request carries the caller-owned connection ID, requires a connected network, uses WorkManager's 15-minute minimum interval, and receives the stable weight-sync tag. It also proves scheduling uses a connection-scoped unique name with `ExistingPeriodicWorkPolicy.UPDATE`, while logout cancellation targets all weight-sync work through that tag.
- `WeightSyncScheduleActionTest` proves startup session checking does not erase durable work, a confirmed logged-out session cancels it, and only an authenticated Ready connection with granted background access is scheduled. Unresolved connections and missing/unsupported background access leave existing work unchanged.
- `WearableConnectionViewModelTest` proves background access is optional capability state on an otherwise-ready foreground connection; grant/denial updates do not create another backend registration request.
- `SharedPreferencesWeightSyncCursorStoreTest` runs against Android private storage and proves cursors survive store recreation, remain isolated by connection ID, return `null` when missing, and remove wrong-typed corrupted values safely.
- `InitialWeightSyncViewModelTest` proves sync is explicit rather than automatic, completed receipts become aggregate counts, repeated taps do not overlap, no-data remains non-error state, interrupted work exposes only counts/recovery reason, and logout cancels and clears previous-user state.
- Isolated `LoginScreen` Compose tests cover session-checking, blank, submitting, safe-error, authenticated-success, logout-click, password visibility, explicit weight sync, and the optional background-permission action. Unsupported background access leaves manual sync visible and hides the unusable permission action.
