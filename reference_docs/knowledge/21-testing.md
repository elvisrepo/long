...
- MockWebServer and repository tests prove the repository sends `POST /api/auth/mobile/login/` with the exact Django JSON body and stores both tokens before returning success. Restoration coverage proves it posts the stored refresh token to `/api/auth/mobile/refresh/`, replaces access, retains or rotates refresh correctly, skips networking without stored tokens, clears tokens after `401`, and preserves tokens across transient network failure. Logout coverage proves the client posts the stored refresh token to `/api/auth/mobile/logout/`, clears locally after successful revocation, and retains tokens after a server failure so revocation can be retried.
- Isolated `LoginScreen` Compose tests cover session-checking, blank, submitting, safe-error, authenticated-success, logout-click, and password-visibility-toggle states. The visibility test proves the user control transitions between Show and Hide while the field retains Android password semantics for keyboards, autofill, and accessibility.
...
- The previous eight instrumented tests pass on the physical `FCP-N49` phone. The two new session-checking/logout Compose tests compile, but still require a reconnected device before the ten-test instrumented suite can be reported green.
...
- The HTTP repository uses a local fake server in JVM tests, the production token store is verified independently on-device, and the real Activity-to-ViewModel wiring is covered on-device. A manual physical-device run also proved live Django login returned `200`, encrypted both JWTs, and rendered `Signed in`. Mock-server tests now prove refresh/expiry rejection behavior; a live phone-to-Django refresh remains to be manually observed. Wearable connection registration, Health Connect access, and uploads remain unproven on-device.
