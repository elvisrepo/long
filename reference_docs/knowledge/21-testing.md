...
- Isolated `LoginScreen` Compose tests cover session-checking, blank, submitting, safe-error, authenticated-success, logout-click, password visibility, explicit weight sync, a disabled cooldown action, and the optional Pro background-permission action. Free/manual-only and unsupported background access hide the unusable background action.
...
- The focused `LoginScreenTest` suite contains 12 tests; the new cooling-down action test passes on the physical `FCP-N49` phone and proves a disabled manual-sync button cannot forward another callback.
