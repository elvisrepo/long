...
- On 2026-07-15, `docker compose exec web uv run pytest -q` passed with `191` backend tests, `uv run ruff check` passed, and repository-wide `uv run mypy` passed across `78` source files.
...
- These fixes and the subsequent Health Connect provider/lifecycle migrations were verified with the complete backend gate: `ruff`, repository-wide `mypy`, `191` pytest tests, migration drift check, and `git diff --check`.
