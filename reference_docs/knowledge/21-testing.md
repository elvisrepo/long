...
- Local JVM tests cover login form state, login and mobile-refresh request/response/error serialization, safe diagnostic strings, ViewModel success/failure/session-restoration state, and the HTTP repository contract. Refresh-contract tests prove the request contains exactly `refresh`, the response requires `access`, rotated `refresh` is optional, and neither token appears in diagnostic strings.
- MockWebServer and repository tests prove the repository sends `POST /api/auth/mobile/login/` with the exact Django JSON body and stores both tokens before returning success. Restoration coverage proves it posts the stored refresh token to `/api/auth/mobile/refresh/`, replaces access, retains or rotates refresh correctly, skips networking without stored tokens, clears tokens after `401`, and preserves tokens across transient network failure.
