...
- The `WearableConnection` model/API foundation, idempotent `SyncRun` receipt boundary, nullable `MetricEntry.source_connection` foreign key, database-backed external-record uniqueness, isolated entry/batch validation, canonical payload hashing, and the isolated synchronous-ingestion happy path are implemented. Same-payload retry reuse, different-payload conflict rejection, and duplicate external-record skipping are next; only then should normalized entries be exposed through the endpoint.
...
- The first ingestion-service test proves one validated new batch atomically stores its canonical hash, creates its `MetricEntry`, marks the `SyncRun` succeeded with one imported entry, and moves the connection from pending to connected with `last_synced_at`.
