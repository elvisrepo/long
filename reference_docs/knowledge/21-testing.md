...
- Wearable-entry serializer tests prove normalized instantaneous `body_weight` acceptance, configured range and finite-number enforcement, active supported system-definition lookup, Samsung Health source restriction, timestamp parsing, and required nonblank external IDs. Steps-specific missing/invalid interval validation remains the next focused TDD scenario. This serializer is the live nested-entry boundary.
