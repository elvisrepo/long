...
- Appearance coverage includes Light and Sand layouts at 320/768/1440px, their auth and
  ...
  all three preferences and cross-tab storage events. Sand persistence is checked
  across reloads and both authentication pages. A chart component test proves an
...
- backend password-reset integration tests prove existing-account delivery, unknown-account non-disclosure, one-use tokens, Django password-policy enforcement, refresh-token revocation, and the three-requests-per-hour throttle
- reset email delivery is intentionally local-only in this slice; tests use Django's in-memory mailbox while the development server prints messages to its console
...
- `metric-entries-api.test.ts` covers `createMetricEntry()`, `getMetricEntries()`, `updateMetricEntry()`, and `deleteMetricEntry()` request shape, auth-token requirements, the nullable `period_start` response contract, backend failure behavior, filter query-string generation including `limit`, and backend validation-detail preservation for entry updates.
...
- Upload API tests prove authentication, owner/active scoping, malformed UUID and missing-entry rejection, synchronous normalized Weight persistence, Steps interval/count persistence, Sleep duration/session persistence with `period_start < recorded_at`, an exact retry with `200`, both conflict types with safe `409` detail, and cross-upload identical-record skipping with terminal counters.
...
- `HealthConnectSleepSampleTest` proves session duration falls back to start/end when stages are absent and subtracts explicit awake, awake-in-bed, and out-of-bed stages when present.
- `AndroidHealthConnectSleepReaderTest` proves `SleepSessionRecord` and stage mapping through an explicit ascending read window. `AndroidHealthConnectMetricAccessTest` proves Weight, Steps, and Sleep grants are all required by the supported metric permission set.
...
- `IncrementalSleepSyncPlannerTest` proves the same shared cursor window and Samsung-origin filtering for complete sleep sessions.
