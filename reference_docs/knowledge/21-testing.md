...
- Isolated `LoginScreen` Compose tests cover session-checking, blank, submitting, safe-error, authenticated-success, logout-click, password visibility, explicit weight sync, a disabled cooldown action, the optional Pro background-permission action, and forwarding the Ready-state Disconnect action. Free/manual-only and unsupported background access hide the unusable background action.
