...
- The `WearableConnection` model/API foundation, idempotent `SyncRun` receipt boundary, nullable `MetricEntry.source_connection` foreign key, database-backed external-record uniqueness, isolated entry/batch validation, canonical payload hashing, and isolated new/exact-retry/conflict/identical-record-skip ingestion paths are implemented. Mixed new/duplicate batch counters and changed-content external-record behavior are next; only then should normalized entries be exposed through the endpoint.
