...
- `AndroidHealthConnectWeightReaderTest` constructs real AndroidX `WeightRecord` values and proves the adapter uses the requested instant window, requests ascending records, converts mass to kilograms, maps record identity/timestamp/data origin, and follows page tokens without dropping later pages.
...
- `InitialWeightSyncCoordinatorTest` proves one planned batch receives one generated identity, no-data creates neither an identity nor a request, 101 records upload as ordered `100 + 1` batches with distinct UUIDs, and a later failure stops processing while retaining earlier committed receipts.
