...
- On 2026-07-15, `docker compose exec web uv run pytest -q` passed with `188` backend tests, `uv run ruff check` passed, and repository-wide `uv run mypy` passed across `76` source files.
...
- These fixes and the subsequent Health Connect provider/uniqueness migrations were verified with the complete backend gate: `ruff`, repository-wide `mypy`, `188` pytest tests, migration drift check, and `git diff --check`.
...
- API tests should prove authentication is required, list and status-detail responses are scoped to the caller, creation stores `request.user`, disconnect removes only the caller's connection and releases its slot, and invalid provider/status values are rejected. Later trusted ingestion-service tests should cover server-managed state updates.
...
- Free-plan zero-limit, duplicate-provider, and client-supplied status rejection are covered. Disconnect coverage proves authentication, owner deletion, cross-user `404`, harmless repeated deletion, and entitlement-slot reuse. Status-read coverage proves the owner response contract, authentication, and cross-user `404`. Additional server-managed-field cases remain on the immediate API test list.
