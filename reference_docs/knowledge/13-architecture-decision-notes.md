...
- Status: Superseded by plan-backed entitlement resolution
- Date: 2026-06-05
- Decision:
  The original implementation enforced a hard-coded limit of 3 active custom metric definitions per user. System default metrics and inactive archived custom metrics did not count.
- Context:
...
- Current application:
  `apps.metrics.limits.validate_active_custom_metric_limit()` still owns the limit check, but the limit now comes from `get_current_subscription_plan(user).active_custom_metric_limit`. `GET /api/v1/metrics/usage/` exposes the same plan-backed count and limit.
- Consequences:
...
- Revisit when:
  No further action for the temporary constant; it has been removed. Revisit entitlement shape when more plan capabilities are enforced.

### ADR-020: Explicit Current Subscription Per User

- Status: Accepted
- Date: 2026-06-09
- Decision:
  Store product entitlements in shared `SubscriptionPlan` rows and seed one canonical active default plan with `code="free"`. Registration must atomically create an active `Subscription` linking the new user to that plan.
- Context:
  Explicit assignment makes the current plan directly queryable and avoids ambiguous users with no entitlement state. Free subscriptions have no billing provider identifiers.
- Current implementation:
  Migration `0003_seed_free_subscription_plan` creates the free plan. `create_user_with_subscription()` creates the user and free subscription inside `transaction.atomic()`. `get_current_subscription_plan()` resolves the plan from the user's current subscription without a silent fallback.
- Current-subscription rule:
  A conditional database constraint allows at most one subscription in `trialing`, `active`, `past_due`, or `incomplete` state per user. Cancelled rows remain as history.
- Consequences:
