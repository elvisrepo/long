"""Tests for the fixed Systems Manager staging deployment command."""

import pytest

from scripts.staging_ssm import deployment_parameters


BACKEND_IMAGE = (
    "173291122778.dkr.ecr.eu-central-1.amazonaws.com/"
    "syncvitals/staging/backend@sha256:" + "a" * 64
)


def test_deployment_command_captures_rollback_and_uses_host_guards() -> None:
    parameters = deployment_parameters(
        backend_image=BACKEND_IMAGE,
        region="eu-central-1",
    )
    command = parameters["commands"][0]

    assert "docker inspect --format '{{.Config.Image}}'" in command
    assert "previous_backend_image=" in command
    expected_rollback_guard = (
        '[[ "$previous_backend_image" =~ ^173291122778\\.dkr\\.ecr\\.'
        "eu-central-1\\.amazonaws\\.com/syncvitals/staging/"
        "backend@sha256:[0-9a-f]{64}$ ]]"
    )
    assert expected_rollback_guard in command
    assert "python3 -m scripts.staging_runtime" in command
    assert "python3 -m scripts.production_deployment" in command
    assert "export BACKEND_IMAGE=" in command
    assert "http://127.0.0.1:18000/api/v1/health/live/" in command
    assert "http://127.0.0.1:18000/api/v1/health/ready/" in command
    assert "--header 'Host: staging.syncvitals.space'" in command
    assert "--header 'X-Forwarded-Proto: https'" in command
    assert "trap cleanup_ecr_auth EXIT" in command


def test_deployment_command_rejects_a_tagged_image() -> None:
    with pytest.raises(ValueError, match="immutable staging ECR reference"):
        deployment_parameters(
            backend_image=(
                "173291122778.dkr.ecr.eu-central-1.amazonaws.com/"
                "syncvitals/staging/backend:latest"
            ),
            region="eu-central-1",
        )
